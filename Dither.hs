...
-- ditherST ls img@(Image { imageWidth  = w, 
--                                  imageHeight = h, 
--                                  imageData   = vec }) =
--   Image w h pixels
--     where compCount = 3
...
--                             | otherwise = do
--                                 newArr V.// updateVals (findPix ls) errBases []
--                                 colMapper (readIdx  + compCount) (writeIdx + compCount) (x + 1)
--                                     where
--                                     -- calculate indices where a pixel starts (= base) and check for bounds
--                                     baseId :: Point -> Maybe BasID
--                                     baseId (a,b)
...
--                                       where 
--                                       newPix = colorMinDist oldPix pxls
--                                       oldPix = unsafePixelAt vec baseA
--                                       pixErr = subtPixel oldPix newPix
--                                     -- calculate error distribution bases
--                                     errBases :: [(ErrorFac, Maybe BasID)]
--                                     errBases         = [(7,(baseId (x+1,y  )))
